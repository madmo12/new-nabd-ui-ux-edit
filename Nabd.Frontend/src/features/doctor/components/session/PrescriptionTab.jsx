...
import { FaPills, FaPrescriptionBottleAlt, FaPlus, FaTrash, FaSave, FaTerminal, FaSearch, FaClipboardList } from 'react-icons/fa';
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-8 pb-10">
      ...
      <div className="w-full xl:w-[450px] space-y-6">
         <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white min-h-[500px] flex flex-col shadow-2xl shadow-slate-900/10">
            <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-6">
               <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-black uppercase tracking-tight">الروشتة الحالية</h4>
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{medications.length} Medications listed</span>
                  </div>
               </div>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto no-scrollbar max-h-[400px]">
               {medications.length === 0 ? (
                 <div className="h-full flex flex-col items-center justify-center opacity-30 py-20">
                    <FaPrescriptionBottleAlt className="text-4xl mb-4" />
                    <p className="text-[10px] font-black uppercase tracking-widest">No medications added yet</p>
                 </div>
               ) : (
                 medications.map((med, index) => (
                   <div key={med.id} className="group relative bg-white/5 border border-white/5 rounded-2xl p-4 hover:bg-white/10 transition-all">
                      <div className="flex items-start justify-between gap-4">
                         <div className="flex-1">
                            <h5 className="text-xs font-black mb-2 flex items-center gap-2">
                               <span className="text-[#0070CD]">{index + 1}.</span>
                               {med.medicationName}
                            </h5>
                            <div className="flex flex-wrap gap-2">
                               <span className="px-2 py-1 bg-white/5 rounded-md text-[9px] font-black text-slate-400 uppercase tracking-widest border border-white/5">{med.dosage}</span>
                               <span className="px-2 py-1 bg-white/5 rounded-md text-[9px] font-black text-slate-400 uppercase tracking-widest border border-white/5">{med.frequency}</span>
                               <span className="px-2 py-1 bg-[#0070CD]/20 rounded-md text-[9px] font-black text-[#0070CD] uppercase tracking-widest border border-[#0070CD]/10">{med.durationDays} Days</span>
                            </div>
                            {med.specialInstructions && (
                               <p className="mt-2 text-[9px] text-slate-500 font-bold italic">Note: {med.specialInstructions}</p>
                            )}
                         </div>
                         <button onClick={() => setMedications(medications.filter(m => m.id !== med.id))} className="text-slate-600 hover:text-rose-500 transition-colors">
            ...
            <div className="mt-8 pt-8 border-t border-white/10">
               <button
                 ...
                 className="w-full py-4 bg-white text-slate-900 rounded-2xl font-black uppercase text-[10px] tracking-[.3em] shadow-xl hover:bg-[#0070CD] hover:text-white transition-all transform active:scale-95 disabled:opacity-20"
               >
