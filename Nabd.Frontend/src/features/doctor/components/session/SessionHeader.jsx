...
const SessionHeader = ({
  ...
  return (
    <div className="bg-white border-b border-slate-100 px-8 py-4 flex items-center justify-between sticky top-0 z-[100] shadow-sm">
      {/* Tactical Status & Patient Link */}
      <div className="flex items-center gap-6">
        <button
          onClick={onClose}
          className="w-10 h-10 bg-slate-100 text-slate-400 rounded-xl flex items-center justify-center hover:bg-rose-50 hover:text-rose-500 transition-all border border-slate-100"
          title="خروج"
        >
      ...
      <div className="flex items-center gap-1 bg-slate-50 p-1.5 rounded-[1.25rem] border border-slate-100">
         {tabs.map(tab => {
           const Icon = tab.icon;
