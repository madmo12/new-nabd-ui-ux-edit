...
const ReviewDetailsModal = ({ review, isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden border-2 border-[#E7ECEF]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header - Updated with new colors */}
        <div className="bg-gradient-to-r from-[#1C8B8F] to-[#14666A] p-6 relative">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-280px)]">
          ...
          {review.comment && (
            <div className="mb-6">
              <h3 className="text-lg font-bold text-slate-800 mb-3 flex items-center gap-2">
                <FaComments className="text-[#1C8B8F]" />
                التعليق
              </h3>
          ...
          {review.hasDoctorReply && review.doctorReply && (
            <div className="bg-gradient-to-br from-[#1C8B8F]/10 to-[#14666A]/10 rounded-xl p-5 border-2 border-[#1C8B8F]/30">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-8 h-8 bg-[#1C8B8F] rounded-lg flex items-center justify-center">
                  <FaCheckCircle className="text-white text-sm" />
                </div>
                <div>
                  <h4 className="font-bold text-[#1C8B8F]">رد الطبيب</h4>
                  {review.doctorRepliedAt && (
                    <p className="text-xs text-[#1C8B8F]/70">
        ...
        <div className="border-t-2 border-slate-200 p-4 bg-slate-50">
          <button
            onClick={onClose}
            className="w-full px-6 py-3 bg-[#1C8B8F] hover:bg-[#156f72] text-white font-bold rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl"
          >
            إغلاق
          </button>
        </div>
