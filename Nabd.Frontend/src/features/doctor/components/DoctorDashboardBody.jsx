        ...
        {stats?.map((stat) => (
          ...
          >
            <div className="flex items-center gap-4">
              <div 
                className="w-10 h-10 rounded-xl flex items-center justify-center transition-all bg-white shadow-sm group-hover:bg-[#0070CD] group-hover:text-white"
              >
                {stat.icon && <stat.icon className={`w-4 h-4 ${!stat.color ? 'text-[#0070CD]' : ''}`} style={stat.color ? { color: stat.color } : {}} />}
              </div>
              <div>
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{stat.label}</h4>
                <p className="text-slate-900 font-black text-sm">{stat.value} <span className="text-[10px] text-slate-400 uppercase tracking-tighter">{stat.unit}</span></p>
              </div>
            </div>
            
            {stat.trend && (
              <div className={`text-[9px] font-black px-2 py-1 rounded-md ${stat.trendUp ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50'} uppercase tracking-widest`}>
                {stat.trend}
              </div>
            )}
          </article>
