...
public class MainDiagnosisLocalModel : IMainDiagnosisModel, IDisposable
{
    ...
    public MainDiagnosisLocalModel(ILogger<MainDiagnosisLocalModel> logger)
    {
        ...
        var aiModelsPath = Path.Combine(baseDir, "..", "..", "..", "..", "..", "AIModels", "Main");
        _scriptPath = Path.Combine(aiModelsPath, "predict.py");
        _symptomColsPath = Path.Combine(aiModelsPath, "evidences_map.pkl");
    ...
    public async Task<List<AiModelResult>> DiagnoseAsync(List<string> normalizedSymptoms, int? age = null, string? sex = null)
    {
        ...
        // Use a longer timeout for the first start (30s)
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
        try {
            await _readySource.Task.WaitAsync(cts.Token);
        } catch (OperationCanceledException) {
            _logger.LogWarning("AI Model took too long to load (>30s)");
    ...
    public async Task<List<string>> GetAvailableSymptomsAsync()
    {
        ...
        try
        {
            var jsonPath = Path.Combine(Path.GetDirectoryName(_symptomColsPath)!, "symptoms_schema.json");
            if (File.Exists(jsonPath))
            {
                var json = await File.ReadAllTextAsync(jsonPath);
                using var doc = JsonDocument.Parse(json);
                if (doc.RootElement.TryGetProperty("allowed_symptoms", out var allowed))
                {
                    _cachedSymptoms = allowed.EnumerateArray()
